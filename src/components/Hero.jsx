...
const Hero = () => {
  ...
  const scrollButtonAnimation = useSpring({
    opacity: showScrollButton ? 1 : 0,
    transform: showScrollButton ? 'translate3d(0,0,0) scale(1)' : 'translate3d(0,20px,0) scale(0.8)',
  ...
  return (
    <div 
      ...
      ref={ref}
    >
      {/* Scroll to Top Button - Using liquid-glass-icon styling */}
      <animated.button
        style={scrollButtonAnimation}
        onClick={scrollToTop}
        className="liquid-glass-icon fixed bottom-20 sm:bottom-24 md:bottom-28 right-4 sm:right-6 md:right-8 z-40 text-gray-300 hover:text-white transition-all p-2 sm:p-2.5 md:p-3 rounded-full transform hover:scale-110 duration-300 flex items-center justify-center group touch-manipulation select-none"
        aria-label="Scroll to top"
      >
        <FaArrowUp className="w-5 h-5 sm:w-6 sm:h-6 md:w-7 md:h-7 lg:w-8 lg:h-8 group-hover:translate-y-[-2px] transition-transform" />
      </animated.button>
